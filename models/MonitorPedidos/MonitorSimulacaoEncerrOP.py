...
def EstoqueSKUSimulado(ops):
    ...
    consulta = pd.concat([simulacao,consulta])
    # Realizar o agrupamento groupBy
    print(consulta)

    consulta = consulta.groupby('codProduto').agg({
    "estoqueAtual": 'sum',
...
def MonitorDePreFaturamento(empresa, iniVenda, finalVenda, tiponota,rotina, ip, datainicio,parametroClassificacao,
                            tipoData, Representante_excluir, escolherRepresentante, arrayOPSimuladas ):
    ...
    if arrayOPSimuladas != '':
        estoque = EstoqueSKUSimulado(arrayOPSimuladas)
    else:
        estoque = EstoqueSKU()
...
# função para verificar a presença de "casa" no valor da coluna "produto"
    ...
    pedidos1 = Ciclo2(pedidos1, avaliar_grupo,arrayOPSimuladas)
...
def API(empresa, iniVenda, finalVenda, tiponota,rotina, ip, datainicio,parametroClassificacao, tipoData,arrayRepres_excluir, arrayRepre_Incluir,ops):
    tiponota = '1,2,3,4,5,6,7,8,10,24,92,201,1012,77,27,28,172,9998,66,67,233,237'#Arrumar o Tipo de Nota 40
    pedidos = MonitorDePreFaturamento(empresa, iniVenda, finalVenda, tiponota,rotina, ip, datainicio,parametroClassificacao,tipoData, arrayRepres_excluir, arrayRepre_Incluir,ops)
...
def Ciclo2(pedidos,avaliar_grupo,arrayOPSimuladas):
